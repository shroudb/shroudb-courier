//! Parsed RESP3 response types and typed result structs for Courier.
...
impl DeliverResult {
    pub fn from_response(resp: Response) -> Result<Self, ClientError> {
        check_map_response(&resp, "DELIVER")?;
        Ok(Self {
            delivery_id: require_string(&resp, "delivery_id", "DELIVER")?,
            channel: require_string(&resp, "channel", "DELIVER")?,
            adapter: require_string(&resp, "adapter", "DELIVER")?,
            status: require_string(&resp, "status", "DELIVER")?,
            delivered_at: require_int(&resp, "delivered_at", "DELIVER")?,
            error: resp.get_string_field("error"),
        })
    }
}

/// Result from a TEMPLATE_LIST command.
#[derive(Debug, Clone)]
...
impl TemplateListResult {
    pub fn from_response(resp: Response) -> Result<Self, ClientError> {
        check_map_response(&resp, "TEMPLATE_LIST")?;
        let count = require_int(&resp, "count", "TEMPLATE_LIST")?;
        let templates = resp
            .get_field("templates")
            .and_then(|v| match v {
            ...
            .unwrap_or_default();
        Ok(Self { count, templates })
    }
}

/// Result from a TEMPLATE_INFO command.
#[derive(Debug, Clone)]
...
impl TemplateInfoResult {
    pub fn from_response(resp: Response) -> Result<Self, ClientError> {
        check_map_response(&resp, "TEMPLATE_INFO")?;
        Ok(Self {
            name: require_string(&resp, "name", "TEMPLATE_INFO")?,
            has_subject: resp
                .get_string_field("has_subject")
                .is_some_and(|s| s == "true"),
            has_html_body: resp
                .get_string_field("has_html_body")
                .is_some_and(|s| s == "true"),
            has_text_body: resp
                .get_string_field("has_text_body")
                .is_some_and(|s| s == "true"),
        })
    }
}
