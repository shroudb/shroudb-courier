//! Authentication and authorization for Courier connections.
//!
...
mod tests {
    ...
    fn wildcard_allows_all_commands() {
        ...
        assert!(policy.allows_command("DELIVER"));
        assert!(policy.allows_command("TEMPLATE_LIST"));
    }

    #[test]
    fn specific_commands_only() {
        let policy = make_policy(vec!["DELIVER", "TEMPLATE_LIST"]);
        assert!(policy.allows_command("DELIVER"));
        assert!(policy.allows_command("TEMPLATE_LIST"));
        assert!(!policy.allows_command("TEMPLATE_RELOAD"));
    }
